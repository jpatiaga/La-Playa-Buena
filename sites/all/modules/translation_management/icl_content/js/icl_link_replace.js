...
function on_save_click() {
        ...
        jQuery.ajax({
                type: "POST",
                url: Drupal.settings.ican_ajax.ican_link_replace_url,
                data: "icl_link_cmd=icl_save&icl_id=" + id + "&icl_file_name=" + jQuery("#" + text_id).val(),
                async: false,
...
function on_delete_click() {
        ...
        jQuery.ajax({
                type: "POST",
                url: Drupal.settings.ican_ajax.ican_link_replace_url,
                data: "icl_link_cmd=icl_delete&icl_id=" + id,
                async: false,
                success: function(msg){
                        if (msg.split('|')[0] == '1') {
