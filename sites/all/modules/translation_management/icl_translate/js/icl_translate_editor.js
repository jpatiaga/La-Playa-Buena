// $Id: icl_translate_editor.js,v 1.13 2010/09/03 14:40:56 jozik Exp $

jQuery(document).ready(function() {
  
  // Bind focus to fields
  // Show/hide original content
  jQuery('textarea, input[id^=edit-field-data-translated]').bind('focus', function() {
    var hide = jQuery(this).parents('fieldset').find('.original_data');
    if (hide.is(':hidden')) {
      jQuery('.original_data:visible').slideUp();
      hide.slideDown();
    }
  });
  ...
  jQuery('#icl-translate-editor .finished').click(function() {
    iclAjaxSubmit('#icl-translate-editor');
    
    // If unchecked focus to it's field
    if (jQuery(this).is(':checked') == false) {
       jQuery(this).parents('fieldset').find('[id^=edit-field-data-translated]').focus();
       jQuery('#icl-translate-editor-finished').attr('disabled', 1);
       jQuery('#icl-translate-editor-finished').attr('checked', 0);
    // Else find next field
    } else {
      var fieldset = jQuery(this).parents('fieldset').next('fieldset');
  ...
  jQuery('.icl_translate_editor_load_autosave').click(function() {
    var toLoad = jQuery(this).children('div');
    var toLoadData = toLoad.html();
    var toReplace = jQuery(this).parent().children().find('[id^=edit-field-data-translated]');
  ...
  jQuery('[id^=edit-body-field-data-translated-format]').find('input').each(function() {
  ...
  // Keyboard
  jQuery('textarea, input[id^=edit-field-data-translated]').bind('keydown', 'ctrl+space', iclTranslateEditorKeyFinish);

  // Prevent unload if form is submitted
  jQuery('#icl-translate-editor').submit(function() {
    window.onbeforeunload = '';
  });

  // Download XLIFF page reload
  jQuery('#edit-download-xliff').click(function() {
...
function iclTranslateEditorCheckFinished() {
  jQuery('#icl-translate-editor .finished').each(function(e) {
    if (jQuery(this).is(':checked') == false) {
      jQuery('#icl-translate-editor-finished').attr('disabled', 1);
      jQuery('#icl-translate-editor-finished').attr('checked', 0);
      iclTranslateFocus(jQuery(this).parents('fieldset').find('[id^=edit-field-data-translated]'));
      return false;
    } else {
      jQuery('#icl-translate-editor-finished').attr('disabled', 0);
      //jQuery('#icl-translate-editor-finished').attr('checked', 1);
    }
  });
}
...
function iclTranslateEditorLeavePage() {
  ...
  window.onbeforeunload = function (evt) {
    ...
    jQuery.ajax({
        ...
        data: {icl_translator_ajx_action:'translator_remove', uid:Drupal.settings.icl_editor_unload.uid, rid:Drupal.settings.icl_editor_unload.rid},
        cache: false,
        dataType: 'json',
...
function iclTranslateEditorKeyFinish() {
  jQuery(this).parents('fieldset').find('.finished').attr('checked', 1);
  iclTranslateEditorCheckFinished();
}
